...
router.get("/all", authMiddleware, authorize(["admin"]), (req, res) => {
  ...
  db.query(SELECT_ALL_TRAINS_QUERY, (err, data) => {
    if (err) {
      console.log(err);
      return res.json(err);
    }
...
router.get("/get/:id", authMiddleware, authorize(["admin"]), (req, res) => {
  ...
  db.query(SELECT_ALL_TRAINS_QUERY, [id], (err, data) => {
    if (err) {
      console.log(err);
      return res.json(err);
    }
...
router.post("/create", authMiddleware, authorize(["admin"]), (req, res) => {
  ...
  const INSERT_TRAIN_QUERY =
    "INSERT INTO train (numero_train, capacite, classe) VALUES (? , ? , ?)";
...
router.delete(
  ...
  (req, res) => {
    ...
    db.query(DELETE_TRAIN_QUERY, [id], (err, data) => {
      if (err) {
        console.log(err);
        return res.json(err);
      }
...
router.patch("/edit/:id", authMiddleware, authorize(["admin"]), (req, res) => {
  ...
  const UPDATE_TRAIN_QUERY =
    "UPDATE train SET numero_train = ? , capacite = ? , classe = ? WHERE train_id = ?";
