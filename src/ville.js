...
router.patch("/edit/:id", authMiddleware, authorize(['admin']), (req, res) => {
    ...
    const UPDATE_VILLE_QUERY = "UPDATE ville SET nom_ville = ? , photo_ville = ? WHERE code_ville = ?";

    db.query(
        UPDATE_VILLE_QUERY,
        [nom_ville, photo_ville, id],
        (err, data) => {
            if (err) {
                console.log(err);
                return res.json(err);
