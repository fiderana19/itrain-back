...
router.get(
  ...
  (req, res) => {
    ...
    db.query(SELECT_ALL_RESERVATIONS_QUERY, [id], (err, data) => {
      if (err) {
        console.log(err);
        return res.json(err);
      }
...
router.post("/create", authMiddleware, authorize(["client"]), (req, res) => {
  ...
  const INSERT_RESERVATION_QUERY =
    "INSERT INTO reservation (date_reservation, nbr_place, utilisateur_id, trajet_id) VALUES (?, ?, ?, ?)";
...
router.delete(
  ...
  (req, res) => {
    ...
    db.query(DELETE_RESERVATION_QUERY, [id], (err, data) => {
      if (err) {
        console.log(err);
        return res.json(err);
      }
...
router.patch(
  ...
  (req, res) => {
    ...
    const UPDATE_RESERVATION_QUERY =
      "UPDATE reservation SET nom_client = ?, email_client = ?, telephone = ?, date_reservation = ?, trajet_id = ?, place_id = ? WHERE reservation_id = ?";
