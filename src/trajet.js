...
router.get(
  ...
  (req, res) => {
    ...
    db.query(SELECT_ALL_TRAJETS_QUERY, [id], (err, data) => {
      if (err) {
        console.log(err);
        return res.json(err);
      }
...
router.post("/create", authMiddleware, authorize(["admin"]), (req, res) => {
  ...
  const INSERT_TRAJET_QUERY =
    "INSERT INTO trajet (date_trajet , gare_depart, gare_arrive, duree_trajet, heure_depart, heure_arrive, billet,  train_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
router.delete(
  ...
  (req, res) => {
    ...
    db.query(DELETE_TRAJET_QUERY, [id], (err, data) => {
      if (err) {
        console.log(err);
        return res.json(err);
      }
...
router.patch("/edit/:id", authMiddleware, authorize(["admin"]), (req, res) => {
  ...
  const UPDATE_TRAJET_QUERY =
    "UPDATE trajet SET date_trajet = ? , gare_depart = ?, gare_arrive = ?, duree_trajet = ?, heure_depart = ?, heure_arrive = ?, billet = ?, train_id = ? WHERE trajet_id = ?";
